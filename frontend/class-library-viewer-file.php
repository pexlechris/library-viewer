...
class Library_Viewer_File extends Library_Viewer_File_Alias
{
	...
	/**
	 * Class constructor, used to register front-end hooks.
	 *
	 * When this object is constructed, we know that a file will be viewed!
	 *
	 * @since 3.0.0
	 */
	...
	{
		$this->init();
		$file_status = $this->call_file_viewer();
	...
	{
		return array('abspath', 'current_viewer', 'file_identifier', 'current_page_url', 'real_shortcode_page_link', 'folder_fake_path_symbols', 'folder_real_path_symbols',
	 ...
	 * @return void
	 */
	protected function Library_Viewer_File__init_globals_before_init_parameters()
	{
		/**
		 * @since 3.0.0 Moved to constructor method.
		 */
	...
	protected function init_global_real_shortcode_page_link()
	{
		...
		$url_suffix = $this->parameters['url_suffix'] ?? '';

		$this->globals['real_shortcode_page_link'] = $this->rtrim($shortcode_page_link, $url_suffix);
	}
	...
	{
		$this->globals['file_abs_path'] = $this->globals['abspath'] . $this->globals['file_real_link'];
	}
	...
	{
		$this->globals['file_folder_abs_path'] = $this->globals['abspath'] . $this->globals['file_folder_real_path'];
	...
	protected function get_file_action()
	{
		...
		elseif( 'php' === $file_extension )
		{
			...
			$action = $this->have_access(
				$have_file_access,
				'view_file',
				'view_file',
				'no_appropriate_capabilities',
				'redirect_to_login'
			);
		}
	...
	protected function view_file__init_file_status()
	{
		...
		if ( false === $mime_type_found ) {// download
			...
			$this->file_status['headers'][] = 'Accept-Ranges: bytes';
			$this->file_status['headers'][] = "Content-Length: $file_size";
	...
	protected function get_supported_mime_types()
	{
		 ...
		 *   });
		 */
		$mime_types = apply_filters( 'lv_mime_types', wp_get_mime_types(), $this->globals);
