...
//		$this->action('init_globals_after_init_parameters', $parameters);
	...
	public function get_current_page_url($url_suffix = '')
	{
		...
		// if is wp-admin URL
		$admin_url = admin_url();
		if( $this->globals['current_viewer'] === 'folder' && $this->ltrim($current_page_url, $admin_url) !== $current_page_url ){
			$admin_url = untrailingslashit($admin_url);
			$pagenow = $this->rtrim( $GLOBALS['pagenow'], '.php' );
			$page = $_GET['page'] ?? '';
			$current_page_url = untrailingslashit( $admin_url . '/' . $pagenow . '/' . $page );
		}
